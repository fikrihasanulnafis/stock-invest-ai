...
def simulate_portfolio(request: PortfolioRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Gagal mengambil data Yahoo Finance: {str(e)}")
    ...
    optimized = minimize(negative_sharpe, num_assets * [1.0 / num_assets], args=(mean_returns, cov_matrix),
    ...
    sharpe_ratio = round(annual_return_log / (portfolio_daily_std * np.sqrt(252))
        if portfolio_daily_std > 0 else 0, 2
    )
