...
const DEFAULT_METRICS = {
  expectedReturn:0, dailyReturn:0, monthlyReturn:0, annualReturn:0,
  annualVolatility:0, sharpeRatio:0,
  varDaily:0, varMonthly:0, varAnnual:0,
  varDailyRp:0, varMonthlyRp:0, varAnnualRp:0,
  varValue:0, varPercentage:0, riskLevel:'-'
};
...
export default function Portfolio() {
  ...
  const [stockInput,    setStockInput]    = useState(() => localStorage.getItem(LS.STOCKS) || '');
  const [budgetInput,   setBudgetInput]   = useState(() => localStorage.getItem(LS.BUDGET) || '');
  const [period,        setPeriod]        = useState(() => localStorage.getItem(LS.PERIOD) || '1y');
  const [isLoading,     setIsLoading]     = useState(false);
  const [portfolioData, setPortfolioData] = useState(() => lsGet(LS.PORTFOLIO, [{ name: 'Belum ada data', value: 100 }]));
  const [actionPlan,    setActionPlan]    = useState(() => lsGet(LS.ACTION, []));
  const [metrics,       setMetrics]       = useState(() => lsGet(LS.METRICS, DEFAULT_METRICS));
  const [liveMarket,    setLiveMarket]    = useState([]);
  const [ihsg,          setIHSG]          = useState(null);
  const [currentTime,   setCurrentTime]   = useState('');

  // ── Persist input ke localStorage ──────────────────────────────────
  useEffect(() => { localStorage.setItem(LS.STOCKS, stockInput); }, [stockInput]);
  useEffect(() => { localStorage.setItem(LS.BUDGET, budgetInput); }, [budgetInput]);
  useEffect(() => { localStorage.setItem(LS.PERIOD, period); }, [period]);

  // ── Waktu ───────────────────────────────────────────────────────────
  useEffect(() => {
    const fmt = () => setCurrentTime(new Date().toLocaleString('id-ID', {
  ...
  const handleSimulate = async () => {
    ...
    try {
      const res  = await fetch('http://localhost:8000/api/simulate', {
        ...
        body: JSON.stringify({ stocks: stockInput, budget: parseFloat(budgetInput), period })
      });
  ...
  // ── Clear semua ─────────────────────────────────────────────────────
  const handleClear = () => {
    setStockInput(''); setBudgetInput(''); setPeriod('1y');
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { title:'Total Nilai Simulasi',    val: budgetInput ? `Rp ${Number(budgetInput).toLocaleString('id-ID')}` : 'Rp 0', color:'#3b82f6', bg:'bg-blue-500', spark:0 },
          { title:'Expected Annual Return',  val: `${metrics.annualReturn ?? 0}%`,     sub:'(1 Year)', color:'#10b981', bg:'bg-emerald-500', spark:1 },
          { title:'Annual Volatility',       val: `${metrics.annualVolatility ?? 0}%`, sub:'(1 Year)', color:'#8b5cf6', bg:'bg-purple-500', spark:2 },
          {
          ...
          <div key={i} className="bg-[#131b2f] border border-slate-700 rounded overflow-hidden flex flex-col">
            <div className={`h-0.5 ${card.bg}`}></div>
            <div className="p-5 flex justify-between items-start">
              <div>
                <h3 className="text-slate-400 text-[15px] font-semibold uppercase tracking-wider mb-2">{card.title}</h3>
                <p className="text-xl font-bold text-white">{card.val}</p>
                {card.sub && (
                  <p className={`text-xs mt-1 ${card.subColor || 'text-slate-600'}`}>
                    {card.sub}
                  </p>
                )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="flex items-center justify-center gap-8">
            <div className="relative w-72 h-72 flex-shrink-0 ml-10">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={portfolioData} innerRadius={85} outerRadius={130}
                  ...
                  <PieTooltip
                    contentStyle={{ backgroundColor:'#0b1121', borderColor:'#334155', borderRadius:'4px', color:'#fff' }}
                    formatter={v => [`${v}%`, '']}
                  />
                </PieChart>
              </ResponsiveContainer>
              ...
              {portfolioData.map((item, idx) => (
                <div key={idx} className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-sm flex-shrink-0" style={{ backgroundColor: COLORS[idx % COLORS.length] }}></div>
                    <span className="text-white text-sm font-semibold">{item.name}</span>
                  </div>
                  <span className="text-slate-300 text-sm font-bold tabular-nums">{item.value}%</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-[#131b2f] border border-slate-700 rounded p-5">
          ...
          <div className="space-y-2.5">
            {[
              { label:'Return Harian (Expected)',   val: metrics.dailyReturn,     valColor:'text-emerald-400' },
              { label:'Return Bulanan (Expected)',  val: metrics.monthlyReturn,   valColor:'text-emerald-400' },
              { label:'Return Tahunan (Expected)',  val: metrics.annualReturn,    valColor:'text-emerald-400' },
              { label:'Annual Volatility (Risiko)', val: metrics.annualVolatility,valColor:'text-purple-400'  },
            ].map((row, i) => (
              <div key={i} className="bg-[#0b1121] border border-slate-800 rounded p-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded border border-slate-700 bg-slate-800/50 flex items-center justify-center flex-shrink-0">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <polyline points="3,18 8,13 13,15 21,7" stroke="#94a3b8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
                      <polyline points="18,7 21,7 21,10" stroke="#94a3b8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
                    </svg>
                  </div>
                  <span className="text-slate-300 text-sm">{row.label}</span>
                </div>
                <span className={`font-bold text-base ${row.valColor}`}>{row.val ?? 0}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-[#131b2f] border border-slate-700 rounded p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div className="flex gap-2">
            <button
              onClick={handleSimulate} disabled={isLoading}
          ...
          <table className="w-full text-sm table-fixed">
            <thead>
              <tr className="border-b border-slate-800">
                <th className="text-[10px] text-slate-500 uppercase tracking-wider pb-3 font-semibold text-left w-1/3">Periode</th>
                <th className="text-[10px] text-slate-500 uppercase tracking-wider pb-3 font-semibold text-center w-1/3">Persentase</th>
                <th className="text-[10px] text-slate-500 uppercase tracking-wider pb-3 font-semibold text-right w-1/3">Estimasi Kerugian</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/50">
              {[
                { label:'Harian',  pct: metrics.varDaily   ?? 0, rp: metrics.varDailyRp   ?? 0 },
                { label:'Bulanan', pct: metrics.varMonthly ?? 0, rp: metrics.varMonthlyRp ?? metrics.varValue ?? 0 },
                { label:'Tahunan', pct: metrics.varAnnual  ?? 0, rp: metrics.varAnnualRp  ?? 0 },
              ].map((row, i) => (
                <tr key={i}>
                  <td className="py-3 text-slate-300">{row.label}</td>
                  <td className="py-3 text-orange-400 font-bold text-center">{row.pct}%</td>
        ...
        <div className="bg-[#131b2f] border border-slate-700 rounded p-5 flex flex-col">
          ...
          <div className="grid grid-cols-2 gap-4 flex-1">
            <div className="bg-[#0b1121] border border-slate-800 rounded p-4 flex flex-col items-center justify-center text-center">
              <p className="text-slate-500 text-xs mb-3">Sharpe Ratio (1 Year)</p>
              <p className="text-3xl font-bold text-white mb-1">{metrics.sharpeRatio ?? '—'}</p>
              <p className="text-slate-600 text-xs">Semakin tinggi semakin baik</p>
            </div>
            <div className="bg-[#0b1121] border border-slate-800 rounded p-4 flex flex-col items-center justify-center text-center">
            ...
            <table className="w-full text-sm table-fixed">
              <thead>
                <tr className="border-b border-slate-700">
                  <th className="text-left text-[10px] text-slate-500 uppercase tracking-wider pb-3 font-semibold w-[15%]">Saham</th>
                  <th className="text-center text-[10px] text-slate-500 uppercase tracking-wider pb-3 font-semibold w-[18%]">Alokasi</th>
                  <th className="text-center text-[10px] text-slate-500 uppercase tracking-wider pb-3 font-semibold w-[24%]">Expected Return (1Y)</th>
                  <th className="text-center text-[10px] text-slate-500 uppercase tracking-wider pb-3 font-semibold w-[25%]">VaR 95% (1M)</th>
                  <th className="text-center text-[10px] text-slate-500 uppercase tracking-wider pb-3 font-semibold w-[25%]">Rekomendasi</th>
                </tr>
              </thead>
                    ...
                    <tr key={i} className="hover:bg-slate-800/20 transition-colors">
                      <td className="py-3">
                        <div className="flex items-center gap-4">
                          ...
                          <span className="text-white font-semibold text-center">{plan.stock}</span>
                        </div>
                      </td>
                      <td className="py-3 text-slate-300 font-semibold text-center">{alloc}%</td>
                      ...
                      <td className="py-3 text-slate-300 text-center">{plan.indVar}%</td>
                      <td className="py-3 text-center">
                        <span className={`text-xs font-bold px-3 py-1 rounded border text-center ${
